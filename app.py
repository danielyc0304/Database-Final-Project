...
def api_login():
    ...
    # 查詢 user_login 表
    res = supabase.table('user_login').select('user_password').eq('user_account', username).execute()
    if not res.data:
    ...
    if bcrypt.checkpw(password.encode('utf-8'), hashed_password.encode('utf-8')):
        ...
        login_res = supabase.table('user_login').select('user_id').eq('user_account', username).execute()
        ...
        role_res = supabase.table('users').select('user_role').eq('user_id', user_id).execute()
        if role_res.data:
            session['role'] = role_res.data[0]['user_role']
            return jsonify({'message': f"Welcome back, {username}!", 'role': session['role']}), 200
        else:
...
def check_username():
    ...
    exist = supabase.table('user_login').select('user_account').eq('user_account', username).execute()
    if exist.data:
...
def api_signup():
    ...
    # 檢查帳號是否已存在
    exist = supabase.table('user_login').select('user_account').eq('user_account', user_account).execute()
    if exist.data:
    ...
    if files and 'avatar' in files:
        ...
        res_upload = supabase.storage.from_('dbfinal-avatars').upload(filename, file_bytes, {"content-type": avatar.mimetype})
        if not res_upload:
...
def add_house():
    ...
    user_res = supabase.table('user_login').select('user_id').eq('user_account', landlord_account).execute()
    ...
    house_data = {
        ...
        'house_status': data.get('house_status'),  # DRAFT, PUBLISHED, RENTED, OFFLINE, ARCHIVED
...
def get_houses():
    ...
    res = supabase.table('house').select('*, address:house_address_id(full_address)').eq('owner_id', user_id).execute()
